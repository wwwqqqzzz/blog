...
export default function FeaturedArticles({
  items,
}: FeaturedArticlesProps): React.ReactNode | null {
  // 筛选出标记为featured的文章
...
function FeaturedArticleCard({
  ...
  return (
    <motion.div
      className="group relative overflow-hidden rounded-lg border border-gray-200 bg-transparent transition-all hover:-translate-y-1 dark:border-white/10"
      ...
      >
        {item.image && (
          <div className="relative h-48 w-full overflow-hidden">
            ...
            />
            <div className="absolute right-2 top-2 rounded-full bg-primary-500 px-3 py-1 text-xs font-medium text-white">
              精选
            </div>
          </div>
        )}
        <div className="flex flex-1 flex-col p-5">
          <h3 className="mb-2 line-clamp-2 text-xl font-bold text-slate-900 transition-colors group-hover:text-[#10B981] dark:text-white">
            {item.title}
          </h3>
