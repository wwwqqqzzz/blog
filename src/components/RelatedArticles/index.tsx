...
export default function RelatedArticles({ maxArticles = 3 }: RelatedArticlesProps): React.ReactNode | null {
  ...
  const relatedArticles = useMemo(() => {
    // 获取当前文章的标签和其他数据
    const currentTags = tags.map(tag => tag.label.toLowerCase())
    ...
    // 筛选掉当前文章
    const otherPosts = allPosts.filter(post => post.title !== title)
    ...
    const postsWithScore = otherPosts.map((post) => {
      ...
      const titleWords = postTitle.split(' ').map(word => word.replace(/[,.?!;:'"()\[\]{}]/g, ''))
      const matchingTitleWords = titleWords.filter(word =>
        word.length > 2 && currentTitle.includes(word),
      )
...
function RelatedArticleCard({ article, index }: RelatedArticleCardProps): React.ReactNode {
  ...
  return (
    ...
    >
      <Link href={article.link} className="block h-full hover:no-underline">
